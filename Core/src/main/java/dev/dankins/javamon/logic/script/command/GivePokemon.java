...
public class GivePokemon extends Command {
	...
	public GivePokemon(final List<String> args) throws ScriptLoadingException {
		...
		try {
			...
			throw new ScriptLoadingException("GivePokemon", SCRIPT_LOADING_ERROR_TYPE.invalidNumberOfArguments);
		} catch (final NumberFormatException e) {
			throw new ScriptLoadingException("GivePokemon", SCRIPT_LOADING_ERROR_TYPE.invalidArgument);
		}
	}
	...
	public Optional<String> execute(final Game game, final Map<String, String> strings,
		...
		final MonsterInstance poke = new MonsterInstance(
				game.getMonsterList().getMonster(parseString(monster, strings)), level, game.getPlayer().getName(),
		...
		final ChoiceboxHandler choiceboxHandler = new ChoiceboxHandler(game,
				"Do you want to give a nickname to " + poke.monster.name, Lists.newArrayList("Yes", "No"));
