...
public class AttackHitByThisTurn extends Targeted implements Evaluator<AttackInstance> {

	@JsonCreator
	public AttackHitByThisTurn(@JsonProperty("target") final Target target) {
		super(target);
	}

	@Override
	public AttackInstance evaluate(final AttackAction attack, final MonsterHandler target) {
		return getTarget(attack, target).moveHitByThisTurn;
	}
